...
export function LocationPickerFixed({
  initialAddress = "",
  initialCoordinates,
  onLocationChange,
    ...
    try {
      ...
      if (coordinates) {
        ...
        marker.on("dragend", (event: any) => {
          ...
          const dragLabel = address?.trim()
            ? address.trim()
            : `${position.lat.toFixed(5)}, ${position.lng.toFixed(5)}`;
          if (!address?.trim()) setAddress(dragLabel);
          onLocationChange(dragLabel, newCoords);
          setGeocodingError("");
        });
      ...
      map.on("click", (event: any) => {
        ...
        if (markerRef.current) {
          ...
          newMarker.on("dragend", (dragEvent: any) => {
            ...
            const dragLabel = address?.trim()
              ? address.trim()
              : `${dragPosition.lat.toFixed(5)}, ${dragPosition.lng.toFixed(5)}`;
            if (!address?.trim()) setAddress(dragLabel);
            onLocationChange(dragLabel, dragCoords);
            setGeocodingError("");
          });
        ...
        setCoordinates(newCoords);
        // Si no se ha escrito dirección, usar las coordenadas como etiqueta
        // para que el campo oculto del formulario padre nunca quede vacío.
        const locationLabel = address?.trim()
          ? address.trim()
          : `${position.lat.toFixed(5)}, ${position.lng.toFixed(5)}`;
        if (!address?.trim()) setAddress(locationLabel);
        onLocationChange(locationLabel, newCoords);
        setGeocodingError("");
      });
  ...
  useEffect(() => {
    if (mapInstanceRef.current && coordinates && isMapLoaded) {
  ...
  }, [coordinates, isMapLoaded, address, onLocationChange]);
  ...
  const handleAddressSubmit = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setGeocodingError(
          "No se pudo encontrar la dirección. Intenta ser más específico o selecciona la ubicación manualmente en el mapa.",
        );
      }
    } catch (error) {
      setGeocodingError(
        "Error al buscar la dirección. Por favor, selecciona la ubicación manualmente en el mapa.",
      );
    } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>
          · Ingresa una dirección y haz clic en "Ubicar" para buscar
          automáticamente
        </p>
        <p>
          · O haz clic directamente en el mapa para seleccionar una ubicación
        </p>
